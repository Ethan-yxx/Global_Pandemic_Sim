...
library(data.table)
###########################
#for wuhan simulation
df<-data.frame(read_csv("./data/dfbasewuhan.csv"))
df<-subset(df, !is.na(ci))
names(df)[which(names(df)=="start")]<-"nameu"
df<-subset(df, ssl=="low" & r0 == 2.4 & 
             nameu=="Wuhan_NA_China")

# df<-data.frame(read_csv("./data/dfbasenewyork.csv"))
# df<-subset(df, !is.na(ci))
# names(df)[which(names(df)=="start")]<-"nameu"
# df<-subset(df, ssl=="low" & r0 == 2.4 & 
#              nameu=="New York_NY_United States")
...
ggsave("./figs/fig1c.jpeg", icplot,  width = 10, height = 10, units = "cm",dpi=1200,bg="transparent")
ggsave("./figs/fig1c.svg", icplot,  width = 10, height = 10, units = "cm",dpi=1200,bg="transparent")
